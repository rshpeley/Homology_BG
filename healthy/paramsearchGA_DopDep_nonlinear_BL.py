...
def paramSearch(d):
	...
	elmt1 = pickle.load(open(path+"uniqueLastIterBL.pickle","r")) 			
...
#			d2ti = unknownparams['d2ti'][ind[15]]
			...
			if 1.5*np.abs(d1ti) >= np.abs(d1ta)  or 1.5*np.abs(d2ti) >= np.abs(d2ta) or np.abs(stnta) >= np.abs(stnti) or np.abs(tid2) >1. or np.abs(tad2) > 1. or np.abs(d1ta) > np.abs(d2ta):
				continue
			...
			if np.mean(SWARates['d1'][1000:]) > 0.1 and np.mean(SWARates['d2'][100./p.params["dt"]:]) > 0.1 and np.mean(SWARates['fsi'][1000:]) > 0.1 and 			np.mean(SWARates['ta'][1000:]) > 0.5 and np.mean(SWARates['ti'][1000:]) > 0.5 and np.mean(SWARates['stn'][1000:]) > 0.5 and np.mean(SWARates['gpi'][100./p.params["dt"]:]) > 0.1:
				tests[6] = 1
		...
		if epoch%1 == 0 and len(nextgen) > 0:
			pickle.dump(nextgen,open(path+"Combinations_BL"+str(epoch+1883)+".pickle","w"))
