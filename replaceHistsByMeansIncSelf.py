...
def replace(params):
	...
	samplesH = []
	samplesChangedH = []		
	# Temporary commenting !! Needed healthy results urgently
	'''
	...
	for i,k in enumerate(randSampsPD):	
		...
		for l,x in enumerate(indexOrdPD):
			if x < 16:
				A[ids[x][0],ids[x][1]] = MeanPD[x][0]  # Replacing by 1, coul dbe replaced by either Mean2 , since K-S statistic is very small
			else:
	...
	for i,k in enumerate(randSampsH):
		...
		for l,x in enumerate(indexOrdH):
			if x < 16:
				A[ids[x][0],ids[x][1]] = MeanH[x][0]  # Replacing by 1, coul dbe replaced by either Mean2 , since K-S statistic is very small
			else:
	...
	pickle.dump(SamplesAllH,open(pathname1+"RHBM_H_"+connStr+"_Self_Inc_Sep.pickle","w")) # Just store whatever is done so far
