...
for i,clusPD in enumerate(clusterIndsPD):
	if len(clusPD) > 0 :
		...
		fh.write('import analysisFFT_BL as aFFT\n')
		fh.write('aFFT.analFFT(%s)\n'%(params))
		fh.close()
		...
		'#SBATCH --output=/home/j.bahuguna/homology/Mallet/wojc1jc2/slowNormParams/newParams/strictHealthy/output/FS_%s.out'%(sim_name),
		'#SBATCH --error=/home/j.bahuguna/homology/Mallet/wojc1jc2/slowNormParams/newParams/strictHealthy/output/FS_%s.err'%(sim_name),
		#'#PBS -j oe\n',
		'#SBATCH --job-name=%s\n'%(sim_name),
