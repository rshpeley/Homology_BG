...
clusterIndsPD = pickle.load(open(path5+"clustersPD_BL_5.pickle","r"))
for i,clusPD in enumerate(clusterIndsPD):
	if len(clusPD) > 0 :
		...
		'#SBATCH --output=/home/j.bahuguna/homology/Mallet/wojc1jc2/slowNormParams/newParams/output/CP_%s.out'%(sim_name),
		'#SBATCH --error=/home/j.bahuguna/homology/Mallet/wojc1jc2/slowNormParams/newParams/output/CP_%s.err'%(sim_name),
		#'#PBS -j oe\n',
		'#SBATCH --job-name=%s\n'%(sim_name),
