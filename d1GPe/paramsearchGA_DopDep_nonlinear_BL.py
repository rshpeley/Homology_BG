...
def paramSearch(d,anti=0):
	...
	for epoch in xrange(5000):
		...
		for i,ind in enumerate(elmt):
			...
			if np.mean(SWADopDepRates['d1'][100./p.params["dt"]:]) > 0.1 and np.mean(SWADopDepRates['d2'][100./p.params["dt"]:]) > 0.1 and np.mean(SWADopDepRates['fsi'][100./p.params["dt"]:]) > 0.1 and	np.mean(SWADopDepRates['ta'][100./p.params["dt"]:]) > 1.0 and np.mean(SWADopDepRates['ti'][100./p.params["dt"]:]) > 1.0 and np.mean(SWADopDepRates['stn'][100./p.params["dt"]:]) > 1.0 and np.mean(SWADopDepRates['gpi'][100./p.params["dt"]:]) > 0.1:
				tests[10] = 1
		#	if anti==0:
