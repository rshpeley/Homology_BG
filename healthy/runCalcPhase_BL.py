...
clusterIndsPD = pickle.load(open(path5+"clustersH_BL_5.pickle","r"))
for i,clusPD in enumerate(clusterIndsPD):
	if len(clusPD) > 0 :
		...
		fh.write('import calcPhase_BL as cP\n')
		fh.write('cP.calcPhase(%s)\n'%(params))
		fh.close()
		...
		'#SBATCH --output=/home/j.bahuguna/homology/Mallet/wojc1jc2/slowNormParams/newParams/strictHealthy/output/CP_%s.out'%(sim_name),
		'#SBATCH --error=/home/j.bahuguna/homology/Mallet/wojc1jc2/slowNormParams/newParams/strictHealthy/output/CP_%s.err'%(sim_name),
		#'#PBS -j oe\n',
		'#SBATCH --job-name=%s\n'%(sim_name),
