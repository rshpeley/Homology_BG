...
#AllAsPD_S = pickle.load(open(pathname+"AllAsPDShuff.pickle","r"))
#AllAsPD_S = pickle.load(open(pathname+"AllAsPDShuff1.pickle","r")) # 1,2,3, are different trials, instances of shuffling
...
AllAsH= pickle.load(open(pathname1+"AllAsQ1.pickle","r"))		# Only consider cluster 0, since shuffling among the whole array leads to different values
...
def replace(params):
	...
	'''
	if ind1 < 16:
		print "unshuffled version:PD",np.array(AllAsPD)[:,x1,x2]	
		print "mean,var-unshuffled version:PD",(np.mean(np.array(AllAsPD)[:,x1,x2]),np.var(np.array(AllAsPD)[clusPD,x1,x2]))
		print "shuffled version:PD",np.array(AllAsPD_S)[:,x1,x2]
		print "mean,var-shuffled version:PD",(np.mean(np.array(AllAsPD_S)[:,x1,x2]),np.var(np.array(AllAsPD_S)[:,x1,x2]))

	else:
		print "unshuffled version:PD",np.array(AllBsPD)[:,0,x1]	
		print "mean,var-unshuffled version:PD",(np.mean(np.array(AllBsPD)[:,0,x1]),np.var(np.array(AllBsPD)[clusPD,0,x1]))
		print "shuffled version:PD",np.array(AllBsPD_S)[:,0,x1]
		print "mean,var-shuffled version:PD",(np.mean(np.array(AllBsPD_S)[:,0,x1]),np.var(np.array(AllBsPD_S)[:,0,x1]))
	
	if ind1< 16:

		print "unshuffled version:H",np.array(AllAsH)[:,x1,x2]	
		print "mean,var-unshuffled version:H",(np.mean(np.array(AllAsH)[:,x1,x2]),np.var(np.array(AllAsH)[clusH,x1,x2]))
		print "shuffled version:H",np.array(AllAsH_S)[:,x1,x2]
		print "mean,var-shuffled version:H",(np.mean(np.array(AllAsH_S)[:,x1,x2]),np.var(np.array(AllAsH_S)[:,x1,x2]))
	else:
		
		print "unshuffled version:H",np.array(AllBsH)[:,0,x1]	
		print "mean,var-unshuffled version:H",(np.mean(np.array(AllBsH)[:,0,x1]),np.var(np.array(AllBsH)[clusH,0,x1]))
		print "shuffled version:H",np.array(AllBsH_S)[:,0,x1]
		print "mean,var-shuffled version:H",(np.mean(np.array(AllBsH_S)[:,0,x1]),np.var(np.array(AllBsH_S)[:,0,x1]))
	...
	#for i in xrange(len(AllAsPD))[:1000]:	
	# Commented only temporarily, needed healthy results urgently !! Uncomment
	...
	for i,x in enumerate(randSampsPD):	
		...
		print "ind1",ind1
		if ind1 < 16:
			A[ids[ind1][0],ids[ind1][1]] = AllAsPD_S[x][x1,x2]  # Replacing by each other's mean, if truly interchageable, this wont matter. Replace by its own mean, will lead to 500 identical models.
 		else:
	...
	for i,x in enumerate(randSampsH):
		...
		print "ind1",ind1
		if ind1 < 16:
			A[ids[ind1][0],ids[ind1][1]] = AllAsH_S[x][x1,x2]  # Replacing by 1, coul dbe replaced by either Mean2 , since K-S statistic is very small
 		else:
	...
	pickle.dump(SamplesAllPD,open(pathname+"RHBM_PD_Self_Shuff_"+str(seed1)+"_"+conn+datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d')+".pickle","w"))
	#pickle.dump(SamplesAllH,open(pathname1+"RHBM_H_Self_Shuff_"+str(seed1)+"_"+conn+".pickle","w")) # Just store whatever is done so far
	#pickle.dump(SamplesAllH,open(pathname1+"RHBM_H_Self_Shuff_1_"+str(seed1)+"_"+conn+".pickle","w")) # Just store whatever is done so far
	pickle.dump(SamplesAllH,open(pathname1+"RHBM_H_Self_Shuff_1_"+str(seed1)+"_"+conn+datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d')+".pickle","w")) # Just store whatever is done so far
