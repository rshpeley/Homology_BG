...
def paramSearch(d):
	path = "/home/j.bahuguna/homology/Mallet/wojc1jc2/slowNormParams/strictHealthy/output/"
	...
	elmt = pickle.load(open(path+"uniqueLastIter.pickle","r")) 			
	'''
...
#			d2ti = unknownparams['d2ti'][ind[15]]
			...
			#Flags.append("DopDep") 
			SWARates = calcRates(Flags,delay,A,B,False,ipctx,iptau=15.)

			# Calculate Rates for Activation and Lesion
			Flags = []
			Flags.append("Act")
			#Flags.append("DopDep")
			ActRates = calcRates(Flags,delay,A,B,False,ipctx,iptau=15.)
		...
		#	print "gpi",np.mean(SWARates['gpi'][1000:])
			...
			if GpeSWA > np.mean(SWARates['d1']) and GpeSWA > np.mean(SWARates['d2']):
		        ...
		        if np.mean(SWARates['stn'])<=50 and np.mean(SWARates['d1']) > np.mean(SWARates['d2']) and np.mean(ActRates['d1']) > np.mean(ActRates['d2']): # Additonal constraint since STN firing rates were going very high. Also consult supplementary figure in Mallet 2008
                		tests[8] = 1
