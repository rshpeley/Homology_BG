...
for no in np.arange(0,3700):
	#filename = "Allcombs_Comb_NewSig"+str(no)+"_.pickle"
	filename = "Allcombs_withConds_BLComb_"+str(no)+"_.pickle"
...
pickle.dump(randSamps,open(path5+"randSamps_BL.pickle","w"))
pickle.dump(Allcombs,open(path5+"Allcombs_BL.pickle","w"))
for ip in input_range:
	params = dict()
	params["ipAmp"] = ip
	sim_name = "PDFPD_BL_"+str(ip)
	...
	fh.write('import checkPDFeatures_BL as cPDF\n')
