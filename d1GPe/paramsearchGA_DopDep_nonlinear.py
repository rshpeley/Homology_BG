...
def paramSearch(d,anti=0):
	...
	path = "/home/j.bahuguna/homology/Mallet/wojc1jc2/slowNormParams/newParams/forReview/d1GPe/output/"
	...
	'''
	var = 0.1
	elmt = pickle.load(open(path+"uniqueCombsWithConds.pickle","r"))
	...
	#elmt1 = pickle.load(open(path+"uniqueLastIterTight.pickle","r"))
	...
	for epoch in xrange(5000):
		...
		if newrandom == 1:
			...
			for i in xrange(lenUnknowns):
				temp = np.random.uniform(np.min(unknownparams[terms[i]]),np.max(unknownparams[terms[i]]),numEpochs) 	
				elmt[:,i] = temp.T # rows are no of elements in GA and columns is number of uknown parameters
			# Random half older values
			elmt1 = pickle.load(open(path+"uniqueCombsWithConds.pickle","r"))[:numEpochs/2]
			for i,x in enumerate(elmt1):
				for j,y in enumerate(x):
					if len(np.where(np.array(posInd)==j)[0])>0: # so that inhibitory connection is not replaced by +ve value an dvice versa
						low = 0
						mu = y
						up = np.max(unknownparams[terms[j]])
						sig = 1.
						elmt1[i,j] =  sc_st.truncnorm((low-mu)/sig,(up-mu)/sig,loc=mu).rvs(1)
					elif len(np.where(negInd==j)[0])>0:
						low = np.min(unknownparams[terms[j]])
						mu = y
						up = 0.
						sig = 1.
						elmt1[i,j] = sc_st.truncnorm((low-mu)/sig,(up-mu)/sig,loc=mu).rvs(1)
					else:
						elmt1[i,j] = y # if that happens, just leave the original value
						print " it should never end up here"
					print "get an another random set of elements"
			elmt[:len(elmt1),:] = elmt1
		...
		for i,ind in enumerate(elmt):
			...
			'''
			if np.abs(d1ti) > np.abs(d1ta)  or np.abs(d2ti) > np.abs(d2ta) or  np.abs(fsita) >np.abs(fsiti) or np.abs(stnta) > np.abs(stnti): # To hopefully eep dists similar to original, too many differences
				continue
			...
			Flags.append("Trans")
			#ActDopDepRates = calcRates(Flags,delay,A,B,False,ipctx,iptau=14.)
			TransRates = calcRates(Flags,delay,A,B,False,ipctx,iptau=p.params["iptau"],ipamp=0.0)
			...
			if np.mean(SWADopDepRates['d1'][100./p.params["dt"]:]) > 0.1 and np.mean(SWADopDepRates['d2'][100./p.params["dt"]:]) > 0.2 and np.mean(SWADopDepRates['fsi'][100./p.params["dt"]:]) > 0.1 and	np.mean(SWADopDepRates['ta'][100./p.params["dt"]:]) > 1.0 and np.mean(SWADopDepRates['ti'][100./p.params["dt"]:]) > 1.0 and np.mean(SWADopDepRates['stn'][100./p.params["dt"]:]) > 1.0 and np.mean(SWADopDepRates['gpi'][100./p.params["dt"]:]) > 0.1:
				tests[10] = 1
		#	if anti==0:
...
#		print "no11ss",no11s
		...
		if epoch%1 == 0 and len(nextgen) > 0:
			pickle.dump(nextgen,open(path+"Combinations_new_WithConds"+str(epoch+200)+".pickle","w"))
...
#	print "ip",ip
	...
	z4 = Rates[3]*unknownparams['tata'] + Rates[4]*unknownparams['tati'] + Rates[5]*unknownparams['tastn'] + Rates[1]*unknownparams['tad2']+ Rates[0]*unknownparams['tad1'] #+Rates[7]*0.1
	...
	z5 = Rates[3]*unknownparams['tita'] + Rates[4]*unknownparams['titi'] + Rates[5]*unknownparams['tistn'] + Rates[1]*unknownparams['tid2']+ Rates[0]*unknownparams['tid1']#+Rates[7]*0.1	#dtibydt = leak1*Rates[4]+S(z5,0.5,100)
...
def calcRates(Flags,d,A,B,fourier,ipctx1,ipamp=5.0,ipfreq=2.,iptau=1,scale=1):
	...
	unknownparams['tid1'] = A[4,0]*scale
	unknownparams['tad1'] = A[3,0]*scale
