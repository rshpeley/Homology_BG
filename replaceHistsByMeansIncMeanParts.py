...
def replace(params):
	...
	seed1 = params["seed"]	
	AllAsPD_S = pickle.load(open(pathname+"AllAsPDShuff_"+str(seed1)+".pickle","r"))[start:end] # 1,2,3, are different trials, instances of shuffling
	AllBsPD_S = pickle.load(open(pathname+"AllBsPDShuff_"+str(seed1)+".pickle","r"))[start:end]

	AllAsH_S= pickle.load(open(pathname1+"AllAsHShuff_"+str(seed1)+".pickle","r"))[start:end]
	AllBsH_S= pickle.load(open(pathname1+"AllBsHShuff_"+str(seed1)+".pickle","r"))[start:end]

	MeanAPD = np.mean(AllAsPD_S,axis=0)
	print "MeanAPD",MeanAPD
	MeanBPD = np.mean(AllBsPD_S,axis=0)
 	MeanAH = np.mean(AllAsH_S,axis=0)
	MeanBH = np.mean(AllBsH_S,axis=0)

	# Make a histogram for all parameters, so that it can be checked that indeed the histograms of the sub samples are similar
	histYsPD = []
	histXsPD = []
	histYsH = []
	cluster1 = []
	...
	As = np.array(AllAsPD_S)
	Bs = np.array(AllBsPD_S)
	for A,B in zip(As,Bs):
		cluster1.append(np.hstack((A,B.T)))

	As = np.array(AllAsH_S)
	Bs = np.array(AllBsH_S)
	for A,B in zip(As,Bs):
	...
	for i in xrange(len(terms)):
		if i < 14:
			#temp1 = np.array(clusPDShuff)[:,ids[i][0],ids[i][1]]
	...
	for i in xrange(len(AllAsPD_S)):	
		temp = dict()
		A = np.array(AllAsPD)[start:end][i]
		B = np.array(AllBsPD)[start:end][i]
		for l,x in enumerate(indexOrdPD):
			if x < 16:
				A[ids[x][0],ids[x][1]] = MeanAPD[ids[x][0],ids[x][1]]  # Replacing by 1, coul dbe replaced by either Mean2 , since K-S statistic is very small
			else:
				B[0,ids[x][0]] = MeanBPD[0,ids[x][0]]
	...
	for i in  xrange(len(AllAsH_S)):
		temp1 = dict()
		A = np.array(AllAsH)[start:end][i]
		B = np.array(AllBsH)[start:end][i]
		print "Before replacing"

		SWA,Act,Trans,Grades = checkValidityH(A,B)	
		for l,x in enumerate(indexOrdH):
			if x < 16:
				A[ids[x][0],ids[x][1]] = MeanAH[ids[x][0],ids[x][1]]  # Replacing by 1, coul dbe replaced by either Mean2 , since K-S statistic is very small
			else:
				B[0,ids[x][0]] = MeanBH[0,ids[x][0]]
	...
	SamplesAllPD["histYsPD"] = histYsPD
	SamplesAllPD["histXsPD"] = histXsPD
	...
	pickle.dump(SamplesAllPD,open(pathname+"RHBM_PD_"+connStr+"_Mean_Inc_Step_"+str(seed1)+str(start)+".pickle","w"))
	pickle.dump(SamplesAllH,open(pathname1+"RHBM_H_"+connStr+"_Mean_Inc_Step_"+str(seed1)+str(start)+".pickle","w")) # Just store whatever is done so far
