...
def replace(params):
	...
	for samp in clusPD:	
		...
		print "ind1",ind1
		if ind1[0] < 16:
			A[ids[ind1[0]][0],ids[ind1[0]][1]] = MeanH[0]  # Replacing by each other's mean, if truly interchageable, this wont matter. Replace by its own mean, will lead to 500 identical models.
 		else:
			B[0,ids[ind1[0]][0]] = MeanH[0]
	...
	for samp1 in clusH:
		...
		print "ind1",ind1
		if ind1[0] < 16:
			A[ids[ind1[0]][0],ids[ind1[0]][1]] = MeanPD[0]  # Replacing by 1, coul dbe replaced by either Mean2 , since K-S statistic is very small
 		else:
			B[0,ids[ind1[0]][0]] = MeanPD[0]
	...
	pickle.dump(SamplesAllPD,open(pathname+"RHBM_PD_"+conn+".pickle","w"))
	pickle.dump(SamplesAllH,open(pathname1+"RHBM_H_"+conn+".pickle","w")) # Just store whatever is done so far
