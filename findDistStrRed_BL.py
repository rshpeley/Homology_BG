...
def run(prefix,pars):
	...
	for d1ta1,d2ta1,fsita1,fsiti1,tata1,tati1,tastn1,tita1,titi1,tistn1,stnti1,tid21,tad21,jc11,jc21,jfsictx1,jstnctx1,stnta1,d1ti1,d2ti1 in zip(d1taDist,d2taDist,fsitaDist,fsitiDist,tataDist,tatiDist,tastnDist,titaDist,titiDist,tistnDist,stntiDist,tid2Dist,tad2Dist,jc1Dist,jc2Dist,jfsictxDist,jstnctxDist,stntaDist,d1tiDist,d2tiDist):
		#if np.abs(d1ti1) > np.abs(d1ta1)/np.random.uniform(1.2,2.5,1)  or np.abs(d2ti1) > np.abs(d2ta1)/np.random.uniform(1.2,2.5,1)  or np.abs(tid21) < 1.2 or np.abs(tad21) < 1.2  or np.abs(stnta1) > np.abs(stnti1)/np.random.uniform(2.,2.5,1) or np.abs(fsita1) > np.abs(fsiti1)/np.random.uniform(1.2,2.5,1):# This is the 1 1for all corase options so it always fails or np.abs(stnta1) > np.abs(stnti1)/1.5:The last condition stnta and stnti maybe important to reduce the bump at low SO in PD networks. Check PruneoutSeveAkine.py 				
		#	continue	
...
#		Flags.append("DopDep")
		...
		Grades = np.sum(tests)	
		cutoff = 12
		if Grades > cutoff:# If its valid, check for wt criteria
			if np.abs(d1ti1)*np.random.uniform(1.2,2.5,1) < np.abs(d1ta1)  or np.abs(d2ti1)* np.random.uniform(1.2,2.5,1)> np.abs(d2ta1)  or np.abs(tid21) > 1.2 or np.abs(tad21) > 1.2  or np.abs(stnta1)* np.random.uniform(2.,2.5,1) < np.abs(stnti1) or np.abs(fsita1)*np.random.uniform(1.2,2.5,1) < np.abs(fsiti1):
	...
	#if np.abs(d1ti)*np.random.uniform(1.2,2.5,1) <  np.abs(d1ta)  and np.abs(d2ti)*np.random.uniform(1.2,2.5,1) < np.abs(d2ta)  and np.abs(tid2) > 1.2 and np.abs(tad2) > 1.2 and np.abs(stnti) > np.abs(stnta)*np.random.uniform(2.,2.5,1) and np.abs(fsita)*np.random.uniform(1.2,2.5,1) < np.abs(fsiti):# This is the 1 1for all corase options so it always fails or np.abs(stnta1) > np.abs(stnti1)/1.5: 
	...
	cutoff = 12
	if Grades > cutoff:
		if np.abs(d1ti)*np.random.uniform(1.2,2.5,1) <  np.abs(d1ta)  and np.abs(d2ti)*np.random.uniform(1.2,2.5,1) < np.abs(d2ta)  and np.abs(tid2) > 1.2 and np.abs(tad2) > 1.2 and np.abs(stnti) > np.abs(stnta)*np.random.uniform(2.,2.5,1) and np.abs(fsita)*np.random.uniform(1.2,2.5,1) < np.abs(fsiti):
