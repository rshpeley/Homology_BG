...
def paramSearch(d,anti=0):
	...
	path = "/home/j.bahuguna/homology/Mallet/wojc1jc2/slowNormParams/newParams/forReview/stnta_zero/output/"
	...
	terms = ["d1ta","d2ta","fsita","fsiti","tata","tati","tastn","tita","titi","tistn","stnti","tid2","tad2","d1ti","d2ti","jc1","jc2","jfsictx","jstnctx"]		
	posInd = [6,9,15,16,17,18]
	negInd = np.array(list(set(np.arange(0,19))-set(posInd)))
	...
	var = 0.1
	elmt = pickle.load(open(path+"uniqueCombs.pickle","r"))
	#dont need to recheck if these combinations are valid if they are loaded from uniqueCombs. set a flag for this
	noCheck = 1 # Very dangerous, dont forget to set it back to zero if you dont read from unique Combs !!!!!!!!!!!!!
	for i,x in enumerate(elmt):
		for j,y in enumerate(x):
			'''
			if np.abs(y) < 3: # Use truncated normal distribution for uppwr and lower limits on norm
				var = 0.2
			else:
				var = 0.6			# Different variance for different means, bigger numbers will have )
			temp = np.random.normal(y,var,1)
			'''
			if len(np.where(np.array(posInd)==j)[0])>0: # so that inhibitory connection is not replaced by +ve value an dvice versa
				low = 0
				mu = y
				up = 12.
				sig = 2.
				elmt[i,j] =  sc_st.truncnorm((low-mu)/sig,(up-mu)/sig,loc=mu).rvs(1)
			elif len(np.where(negInd==j)[0])>0:
 				low = -6.
				mu = y
				up = 0.
				sig = 1. # Checked offline, covers a wider range
				elmt[i,j] = sc_st.truncnorm((low-mu)/sig,(up-mu)/sig,loc=mu).rvs(1)
			else:
				elmt[i,j] = y # if that happens, just leave the original value
				print " it should never end up here"

	#elmt[:100] = elmt1[np.random.randint(0,len(elmt1),100)]
	...
	for epoch in xrange(5000):
		...
		if newrandom == 1:
			...
			for i in xrange(lenUnknowns):
				temp = np.random.uniform(np.min(unknownparams[terms[i]]),np.max(unknownparams[terms[i]]),numEpochs) 	
				elmt[:,i] = temp.T # rows are no of elements in GA and columns is number of uknown parameters
			print "get an another random set of elements"
		...
		if epoch > 5: # Save the last comination so that we have a seed for next GA, stupid cluster has a limit of 24 hours
			pickle.dump(elmt,open(path+"LastComb_BL.pickle","w"))
		for i,ind in enumerate(elmt):
			...
			'''
			#if np.abs(d1ti) > np.abs(d1ta)  or np.abs(d2ti) > np.abs(d2ta) or  np.abs(fsita) >np.abs(fsiti):
			#	continue
			# Dont add leak here, since the leak term shoudl be outside nonlinear function in the differential equation
			A = np.matrix([[knownparams['d1d1'],knownparams['d1d2'],knownparams['d1fsi'],d1ta,d1ti,0.,0.],[knownparams['d2d1'],knownparams['d2d2'],knownparams['d2fsi'],d2ta,d2ti,0.,0.],[0.,0.,knownparams['fsifsi'],fsita,fsiti,0.,0.],[0,tad2,0.,tata,tati,tastn,0],[0.,tid2,0.,tita,titi,tistn,0.],[0.,0.,0.,knownparams['stnta'],stnti,knownparams['stnstn'],0.],[knownparams['gpid1'],0.,0.,knownparams['gpita'],knownparams['gpiti'],knownparams['gpistn'],knownparams['gpigpi']]])
			#print A
			...
			SWADopDepRates = calcRates(Flags,delay,A,B,False,ipctx,iptau=p.params["iptau"])
			...
			Flags.append("Trans")
			#ActDopDepRates = calcRates(Flags,delay,A,B,False,ipctx,iptau=14.)
			TransRates = calcRates(Flags,delay,A,B,False,ipctx,iptau=p.params["iptau"],ipamp=0.)
			...
			if np.mean(SWADopDepRates['ti']) > np.mean(ActDopDepRates['ti']) and np.mean(SWADopDepRates['ta']) < np.mean(ActDopDepRates['ta']):
				tests[2] = 1
			...
			if np.mean(SWADopDepRates['d1'][100./p.params["dt"]:]) > 0.1 and np.mean(SWADopDepRates['d2'][100./p.params["dt"]:]) > 0.5 and np.mean(SWADopDepRates['fsi'][100./p.params["dt"]:]) > 0.1 and	np.mean(SWADopDepRates['ta'][100./p.params["dt"]:]) > 1.0 and np.mean(SWADopDepRates['ti'][100./p.params["dt"]:]) > 1.0 and np.mean(SWADopDepRates['stn'][100./p.params["dt"]:]) > 1.0 and np.mean(SWADopDepRates['gpi'][100./p.params["dt"]:]) > 0.1:
				tests[10] = 1
		#	if anti==0:
			#if GpeSWADopDep > np.mean(SWADopDepRates['d1']) and GpeSWADopDep > np.mean(SWADopDepRates['d2']):
			#if (np.mean(TransRates["d1"]) > 0 and np.mean(TransRates["d1"]) < 5.) and (np.mean(TransRates["d2"]) > 0 and np.mean(TransRates["d2"]) < 5.) and (np.mean(TransRates["fsi"]) > 5. and np.mean(TransRates["fsi"]) < 10.) and (np.mean(TransRates["ta"]) > 0 and np.mean(TransRates["ta"]) < 5.) and (np.mean(TransRates["ti"]) > 10 and np.mean(TransRates["ti"]) < 50.) and (np.mean(TransRates["stn"]) > 5 and np.mean(TransRates["stn"]) < 15.) and (np.mean(TransRates["gpi"]) > 5.):
			tests[11] = 1
	...
	#	print "no11ss",no11s
		...
		if epoch%1 == 0 and len(nextgen) > 0:
			pickle.dump(nextgen,open(path+"Combinations_BL"+str(epoch+0)+".pickle","w"))
	...
	#	howmany = [np.random.randint(0,lenUnknowns) for i in xrange(numEpochs)]
		...
		# Replace each parameter in each combination by a value near the original value, such that it was a mean of a normal distribution
		for i,x in enumerate(nextgen):
			for j,y in enumerate(x[1]):
				if len(np.where(np.array(posInd)==j)[0])>0: # so that inhibitory connection is not replaced by +ve value an dvice versa
					...
					up = 12.
					sig = 2.
					nextgen[i][1][j] = sc_st.truncnorm((low-mu)/sig,(up-mu)/sig,loc=mu).rvs(1)
				elif len(np.where(negInd==j)[0])>0:
					low = -6.
					mu = y
					up = 0.
					...
					nextgen[i][1][j] = y # if that happens, just leave the original value
...
#	print "ip",ip
	...
	#dtibydt = (1./tau)*leak1*Rates[4]+(1./tau)*S(z5,0.4,135)
 	...
 	z6 = unknownparams['jstnctx']*ip + Rates[4]*unknownparams['stnti'] + Rates[3]*knownparams['stnta'] +Rates[5]*knownparams['stnstn']
