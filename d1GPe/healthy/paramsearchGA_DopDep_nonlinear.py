...
def paramSearch(d):
	path = "/home/j.bahuguna/homology/Mallet/wojc1jc2/slowNormParams/newParams/forReview/d1GPe/healthy/output/"
	...
	for epoch in xrange(4000):
		...
		for i,ind in enumerate(elmt):
			...
			if np.abs(d1ti) > np.abs(d1ta)  or np.abs(d2ti) > np.abs(d2ta) or  np.abs(fsita) >np.abs(fsiti) or np.abs(stnta) > np.abs(stnti):
				continue
			...
			Flags = []
			Flags.append("SWA")
			# To compare Dopamine depletion and healthy states we keep the external parameters same (for eg, jc1 <  jc2, and increased jstnctx)
			# And then decreased tastn,tistn can be interpreted as decreased stn rates duirng healthy states
			#Flags.append("DopDep") 
			#SWARates = calcRates(Flags,delay,A,B,False,ipctx,iptau=14.)
			...
			Flags.append("Act")
			#Flags.append("DopDep")
			#ActRates = calcRates(Flags,delay,A,B,False,ipctx,iptau=14.)
			...
			if np.round(np.mean(SWARates['ti'])) >= 9.5 and np.round(np.mean(SWARates['ti'])) <= 45:# and SWARates['tiFF'] < 1.5:
			...
			if np.mean(ActRates["ti"]) >np.mean(SWARates["ti"]) and np.mean(ActRates["ta"]) > np.mean(SWARates["ta"]):
		        ...
		        if np.mean(SWARates['stn'])<=50 and  np.mean(SWARates['d1']) /np.mean(SWARates['d2']) > 1.5 and np.mean(ActRates['d1']) /np.mean(ActRates['d2']) >1.5  : # Additonal constraint since STN firing rates were going very high. Also consult supplementary figure in Mallet 2008
      				tests[8] = 1
		...
		#indices = np.array([i for i,e in enumerate(nextgen) if e[0] >=8.]) 
		...
		if epoch%1 == 0 and len(nextgen) > 0:
			pickle.dump(nextgen,open(path+"Combinations_new_WithConds"+str(epoch+1900)+".pickle","w"))
...
#	print "ip",ip
	...
	z5 = Rates[3]*unknownparams['tita'] + Rates[4]*unknownparams['titi'] + Rates[5]*unknownparams['tistn'] + Rates[1]*unknownparams['tid2']+ Rates[0]*unknownparams['tid1']#+Rates[7]*0.1
