...
for i in np.arange(0,377,1):
	fname = path5+"Combinations_GSSO"+str(0+i)+".pickle"
...
pickle.dump([seqs,uniqueIds],open("uniqueCombs_GSSO_1.pickle","w"))

Ucombs = pickle.load(open("uniqueCombs_GSSO_1.pickle","r"))
seqs = Ucombs[0]
uniqueIds = Ucombs[1]
...
pickle.dump(batch_pars['ind'],open(path5+"uniqueLastIter_GSSO_1.pickle","w"))
...
for x in np.arange(0,len(batch_pars['ind']),100):
	...
	for jj,ii in enumerate(randSamps):
		...
		fh.write('import findDistStrRed_GSSO as fD\n')
		fh.write('prefix = "%s"\n'%postfix)
