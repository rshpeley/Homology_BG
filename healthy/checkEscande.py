...
clustersH = pickle.load(open(pathname+"clustersH"+str(ipNo1)+".pickle","r"))

labelH = pickle.load(open(pathname+"LabeledClustersH"+str(ipNo1)+".pickle","r"))


gmmH = pickle.load(open(pathname+"gmmHwo"+str(ipNo1)+".pickle","r"))
...
def calcFI(params):
	for i,clus1 in enumerate(clustersH[::1]):
		if len(clus1) > 1:
			...
			meanD2 = []
			ipamp = params["ipAmp"]
			targLabel = labelH[i]["label"]
			corrInds = []
			allLabels =[]
			for j,ind in enumerate(means):
			#if gmmH[i]["labels"][j]["label"] == targLabel: # Only if this is correct functional label
				...
				Rates = psGA.calcRates(Flags,1,A,B,False,ipctx1,ipamp=ipamp)

				meanD1.append(np.mean(Rates['d1']))
				meanD2.append(np.mean(Rates['d2']))
				corrInds.append(j)
				allLabels.append(gmmH[i]["labels"][j]["label"])
			RatesD1[i] =dict()
			#RatesD1[i]["label"] = labelH[i]["label"]
			RatesD1[i]["label"] = allLabels
			RatesD1[i]["F_I"] = meanD1
			RatesD1[i]["corrInds"]=corrInds
			RatesD2[i] =dict()
			#RatesD2[i]["label"] = labelH[i]["label"]
