...
def paramSearch(d,anti=0):
	...
	for epoch in xrange(5000):
		...
		for i,ind in enumerate(elmt):
			...
			if np.round(np.mean(ActDopDepRates['ta'])) >= 7 and np.round(np.mean(ActDopDepRates['ta'])) <=15: 
				tests[7] = 1

			if np.round(np.mean(SWADopDepRates['ta'])) >=1 and np.round(np.mean(SWADopDepRates['ta'])) <=6: 
				tests[8] = 1
			...
			if SWADopDepRates['stn_ta'] > 0:
				tests[4] = 1
		
			# Check if STN is anti-phase with TI
			if SWADopDepRates['stn_ti'] < 0:
				tests[5] = 1 	
